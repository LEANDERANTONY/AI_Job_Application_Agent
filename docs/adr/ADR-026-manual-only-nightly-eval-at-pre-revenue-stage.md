...
1. **No active cron line on the VPS.** The `30 3 * * * docker exec ai-job-application-agent-api python -m backend.nightly_eval --include-llm ...` line stays documented in `docs/deployment.md` and is shown as a commented block in `crontab -e`, but is not active. `crontab -l` on the VPS shows zero `nightly_eval` lines.
...
1. **Uncomment the cron line.** `docs/deployment.md` has the recommended line. Move it out of the documentation block and into an active crontab entry.
...
- **Operational knowledge required.** The operator has to remember to run the eval after meaningful changes. Mitigation: this ADR + the DEVLOG Day 44 entry + the `docs/deployment.md` cron block all flag the pattern.
...
- `docs/deployment.md` — the cron block is documented but commented out
