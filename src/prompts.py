...
_PRODUCT_KNOWLEDGE_BLOCK = (
    ...
    "Resume themes — TWELVE are available to users for export. SIX are single-column and ATS-safe: professional_neutral (the product-wide default), classic_ats, modern_blue, creative_warm, architect_mono, noir_cream. SIX are two-column designer themes with a sidebar: timeline_tech, editorial_minimal, classic_slate, monochrome_black, plum_berry, burgundy_champagne. "
    "The two-column themes are NOT ATS-safe — a sidebar/multi-column layout is the most common cause of résumé-parser failures — so recommend them for applications a human reviews directly, portfolios, or networking, and steer users to a single-column theme for bulk ATS submissions. The two-column themes are gated to Pro and Business (like every theme except professional_neutral); DOCX of a two-column theme is exported as a single-column document. "
